		...
		<article>
			...
			<ul class="pagination pagination">
				<c:choose>
					...
					<c:otherwise>
						<li><a href="/contents/${id }/${name }/?page=${i }">First</a></li>
						<li><a href="/contents/${id }/${name }/?page=${current - 1 }">Prev</a></li>
					</c:otherwise>
				</c:choose>
				...
				<c:forEach begin="${begin }" end="${end }" var="i">
					<c:choose>
						<c:when test="${i == current }">
							<li class="active"><a href="/contents/${id }/${name }/?page=${i }">${i }</a></li>
						</c:when>
						<c:otherwise>
							<li><a href="/contents/${id }/${name }/?page=${i }">${i}</a></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				...
				<c:choose>
					<c:when test="${current == 1 }">
						<li><a href="/contents/${id }/${name }/?page=${current + 1}">Next</a></li>
						<li><a href="/contents/${id }/${name }/?page=${last }">Last</a></li>
					</c:when>
